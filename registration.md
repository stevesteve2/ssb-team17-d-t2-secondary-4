# Silicon Sample Benchmark — method registration form (completed, D_T2_secondary-4)

## 0 · Approach identity and output
- **0.1 Team ★** — team_17 — Steve Rathje (Carnegie Mellon University, srathje@andrew.cmu.edu, ORCID 0000-0001-6727-571X) and Dan-Mircea Mirea (University of Pennsylvania, danmirea@sas.upenn.edu, ORCID 0000-0002-4349-7059). This entry's pipeline was built and run by D.-M. Mirea via an automated Codex CLI agent; the team's other entries (secondary-1/-2 and the primary) use a separate Claude-based pipeline by S. Rathje, developed in parallel from a shared project brief (the pipelines were not fully independent of each other).
- **0.2 Plain-language summary ★** — We ask an OpenAI model, three independent times per condition, to forecast mean survey responses of U.S. adults for every condition and demographic subgroup, conditioned on curated published baselines and effects; medians are calibrated by an externally selected 0.5 multiplier on treatment effects and made coherent with fixed demographic weights. This pipeline's best-validated method (held-out RMSE 1.445pp).
- **0.3 Submission tier & approach family ★** — Tier 2; direct cell-level group forecast; single model (gpt-5.6-sol), k=3 ensemble; literature-conditioned; 0.5 effect calibration.
...
- **0.5 Coverage ★** — 221 main cells + 5,967 moderator cells (17 × 27 × 13). Full coverage confirmed: all 16 interventions and all 13 outcomes (+ control where required).
...
- **D.1 Profile source** — N/A at individual level; Tier-2 moderator cells conditioned on documented group baselines with fixed aggregation weights (method/MODERATOR_WEIGHTS.csv, all 27 levels with provenance).
- **D.2 Profile verbalization** — N/A.
- **D.3 Assignment & weighting** — Weighted moderator-level coherence to condition means using the locked weights; deterministic.

## E · Stimulus and survey administration
- **E.1 Stimulus presentation** — Verbatim condition text per call; one treatment condition per call (anti-anchoring); control fillers described with verbatim examples.
- **E.2 Survey walk-through** — N/A (cell-mean forecasting; 13 outcome batches per condition session with exact wordings/scales and evidence).
- **E.3 Response elicitation** — Strict JSON Schema (main + 27 moderator-level means per outcome).
...
- **G.3 Calibration corrections** — Locked 0.5 multiplier on treatment-minus-control effects (control levels untouched), selected on held-out external validation (calibrated RMSE 1.445pp — best of the three methods; method/EXTERNAL_VALIDATION_RESULTS.md).
...
- **K.3 Computational resources** — 51 target calls (3 sessions × 17 conditions), ~0.9-1.3M input + 0.08-0.18M output tokens; cash $0 (plan allowance), API-equivalent $5-9; ~8-20 min.
